...
public class ServerDemo extends JFrame implements ActionListener {
	...
	public static void main(String[] args) {
		new ServerDemo(); // Hàm tạo đối tượng giao diện chat đã định nghĩa phía
							// dưới

		try {
			serA = new ServerSocket(2222); // Tạo cổng kết nối với server ở cổng
											// 2222
			s = serA.accept(); // Chấp nhận khi có kết nối
			while (true) // Vòng lặp liên tục khi nhận được yêu cầu từ phía
							// client gửi về
			{
				BufferedReader nhan = new BufferedReader(new InputStreamReader(s.getInputStream())); // Luồng
																										// nhận
																										// tin
																										// từ
																										// Client
																										...
																										// biến
																										// này
				while ((chuoi = nhan.readLine()) != null) // Kiểm tra xem có tin
															// để nhận từ Client
															// hay không
				{
					temp += chuoi + "\n"; // Khi tin mới được gửi về sẽ được tự
											// động xuống hàng và gán vào biến
											// tạm
	...
	public ServerDemo() {
		...
		Font f = new Font("Arial", Font.BOLD, 20); // Định nghĩa Font chữ nội
													// dung
		...
		JScrollPane sp = new JScrollPane(content); // Định nghĩa cho phép
													// scrolling trên ô nội dung
